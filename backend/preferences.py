"""
DealBot AI — Preference Learning Module
==========================================

Learns and adapts agent weight preferences (w_price, w_time) based on
the user's past deal approval/rejection patterns.

Tracks which deals the user approved vs rejected, and shifts weight
parameters to better align with observed preferences over time.
"""
...
class PreferenceProfile:
    ...
    def _adapt_weights(self) -> None:
        ...
        Logic:
            - If user tends to approve cheap deals → increase w_price
            - If user tends to approve fast deals → increase w_time
        """
        ...
        price_signal = 1.0 - avg_approved_ratio

        all_deals = self._approved_deals + self._rejected_deals
        max_days = max((d["days"] for d in all_deals), default=30) or 30
        time_signal = 1.0 - (avg_approved_days / max_days)
